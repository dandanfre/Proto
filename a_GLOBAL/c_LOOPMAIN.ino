...
void loop(){  
  ...
  changeColor4(readRot4()); //returnerer en streng for første lys: "red", "green" eller "b
  uint32_t myPattern = makeMyColorArr();   //lager en array av typen uint32_t som brukes til å skrive lysene til showMyPattern
  if(digitalRead(guessButton) == LOW){   //invertesrt pga. PULL UP-mode
    //int numCorrect = guessColor();
    statusDioder(guessColor());
    tries(i,t,e); 
