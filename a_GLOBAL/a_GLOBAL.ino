...
String correctColor[4] = {"red", "red", "red", "red"};           //fargene til motstanders boks,LF
static uint32_t myColors[4];                                      //fargene til min boks, skal være av størrelse 4 til en hver tid
String myColorsString[4];                                         //array for å bruke logikk, dvs. guessColors eller myColorPattern
String myGuessColor[4] = {"red","red","green","red"};             //gjetning på motsanders boks. Denne må sammenliknes med LF
...
const int guessButton = 10;
int t = 0;      // tierplassen
int e = 0;      // enerplassen
int i =0;

// fra tabell binary array conversion, 0 til 9
byte numbers [10]={B00111111, B00000110, B01011011, B01001111, B01100110,
...
//***********GuessColor*********************
//denne bruker guessButton = 10, men denne er definert allerede 
const int led1 = 14;
const int led2 = 15;
const int led3 = 16;
const int led4 = 17;
...
void tries(int& i, int& t, int& e) ;    //leser inn referanser til i, e og t og bruker denne funksjonen til å skrive antall forsøk gjettet til seven segment display(SSD)

void showMyPattern(uint32_t myColors[]); //leser in array av typen uint32_t og skriver myColors til ledlysene for å indikere min farge

int guessColor();  //returnerer antall riktige lys gjettet

void statusDioder(int numCorrect); //skriver score, eller antall lys som er riktig gjettet
